...
from utils import process_noise_data
...
def loadcal_main(date:str = LOAD_DATE, **kwargs) -> list[str]:
    ...
    if(graph):
        ...
        ax1.plot(fft_freq / 1e9, ch1_avg_psd, label='Ch1 PSD', color=COLORS[0])
        ax1.plot(fft_freq / 1e9, ch2_avg_psd, label='Ch2 PSD', color=COLORS[1])
        ...
        plt.title('Load Calibration: PSDs and Phase Difference')
        plt.tight_layout()
        plt.show()
    ...
    filepaths.append(os.path.join(OUT_DIR, f"LoadCal.csv"))
